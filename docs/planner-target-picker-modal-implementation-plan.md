...
새 모달은 항목을 시각적으로 비교하고, 임시 선택과 적용을 분리해 최종 대상을 명확하게 바꾸는 것을 목표로 한다.
...
| 참고 UI | 재사용할 패턴 |
| --- | --- |
| `renderPlannerSettingsModal()` | `bg-black/60`, `backdrop-blur-sm`, `rounded-xl`, 헤더/본문/푸터 구분, 취소/확인 버튼 |
...
6. 카드 클릭은 임시 선택만 변경하고 `선택 적용`에서 실제 화면 상태를 바꾼다.
...
- 푸터: 현재 임시 선택 요약, 취소 버튼, 인디고 `선택 적용` 버튼, 상단 border
- 배경 클릭과 X/취소/Escape는 적용하지 않고 닫는다.
...
- 현재 적용된 캐릭터에는 `현재 대상` 배지를 표시한다.
- 임시 선택 카드에는 인디고 border, `ring-2`, 연한 인디고 배경, check 아이콘을 함께 표시한다.
- 같은 카드가 현재 대상이면서 임시 선택인 경우 `선택됨` 하나로 통합해 중복 배지를 피한다.
...
- 넓은 카드 그리드와 좌우 정렬 푸터를 사용한다.
- 모달 바깥의 플래너는 dim 처리하되 현재 화면 문맥이 약하게 보이도록 한다.

모바일:

- 패널은 화면 높이를 대부분 사용하고 `max-h-[94vh]`로 완화한다.
- 제목과 닫기 버튼, 상황 등급 필터, 푸터는 스크롤 영역 밖에 둔다.
- 푸터 버튼은 충분한 터치 영역을 확보하고 `선택 적용`을 넓게 표시한다.
...
window.PROJECT_PLANNER_TARGET_PICKER = {
    ...
    selectedId: '',    // 모달을 열 때의 실제 적용값
    draftId: '',       // 카드 클릭으로 바뀌는 임시 선택값
...
- `selectedId`와 `draftId`를 분리한다.
- 모달을 닫을 때 실제 플래너 상태는 변경하지 않는다.
- 적용이 성공하면 picker 상태를 초기화하고 trigger로 포커스를 복원한다.
...
selectPlannerTargetPickerDraft(id)
applyPlannerTargetPickerSelection()
...
```text
트리거 클릭
-> 현재 캐릭터 ID를 selectedId/draftId로 복사
-> 캐릭터 카드 탐색
-> 카드 클릭으로 draftId 변경
-> 선택 적용
-> setCachedPlannerCharacterId(project, draftId)
-> window.PROJECT_PLANNER_SELECTED_CHARACTER_ID = draftId
-> 모달 닫기
...
- `draftId === selectedId`이면 네트워크 재로딩 없이 모달만 닫는다.
- 존재하지 않는 캐릭터 ID는 적용하지 않고 모달 상태 문구로 안내한다.
- 적용 중에는 버튼을 disabled하고 `적용 중...`을 표시해 중복 로딩을 막는다.
...
```text
트리거 클릭
-> 현재 상황 ID를 selectedId/draftId로 복사
-> 상황 카드 탐색
-> 카드 클릭으로 draftId 변경
-> 선택 적용
-> window.PROJECT_PLANNER_SELECTED_SITUATION_ID = draftId
-> 모달 닫기
...
3. Escape는 임시 선택을 적용하지 않고 닫는다.
4. Tab/Shift+Tab은 모달 내부에서 순환하도록 간단한 focus trap을 둔다.
5. 카드는 native `button`으로 만들고 `aria-pressed`로 임시 선택을 알린다.
6. Enter 또는 Space로 카드 선택이 가능해야 한다.
...
9. 적용 오류는 `aria-live="polite"` 상태 영역으로 알린다.
10. 닫을 때 원래 trigger로 포커스를 복원한다.

## 10. 빈 상태와 예외 처리

| 상태 | 표시와 동작 |
| --- | --- |
| 캐릭터 0명 | `등록된 캐릭터가 없습니다.`와 캐릭터 관리 화면 안내, 적용 비활성화 |
| 상황 0개 | `등록된 상황이 없습니다.`와 상황 관리 화면 안내, 적용 비활성화 |
| 대표 이미지 오류 | `image-off` 아이콘 fallback, 카드 선택은 계속 가능 |
| 선택 대상 삭제 | 첫 항목 fallback을 표시하되 사용자가 적용하기 전에는 실제 상태를 바꾸지 않음 |
| 적용 중 재클릭 | 적용 버튼 disabled, 중복 실행 차단 |
...
5. open/close/filter/draft/apply 이벤트를 named export로 추가한다.
...
3. 임시 선택 시각 상태와 선택 요약을 구현한다.
4. light/dark class를 함께 적용한다.

완료 기준:

- 긴 목록에서도 헤더와 푸터가 고정되고 목록만 스크롤된다.
- 현재 대상과 임시 선택을 텍스트/아이콘/테두리로 구분할 수 있다.
...
- 상황 등급 필터를 바꿔도 현재 임시 선택 상태가 유지된다.

### Phase 4. 적용 흐름

1. 캐릭터 적용을 캐시와 기존 비동기 로더에 연결한다.
2. 상황 적용을 기존 setter와 보존 스크롤 렌더에 연결한다.
3. 같은 값 재적용, 삭제된 ID, 중복 클릭을 처리한다.
4. 모달 닫기와 trigger 포커스 복원을 연결한다.

완료 기준:

- 취소하면 대상이 바뀌지 않는다.
- 적용하면 기존 드롭다운과 동일한 최종 상태와 데이터가 로드된다.
...
3. 다른 캐릭터 카드를 눌러도 적용 전에는 뒤의 플래너가 바뀌지 않는다.
4. 취소, X, 백드롭, Escape로 닫으면 기존 캐릭터가 유지된다.
5. `선택 적용` 후 캐릭터 meta, 이미지 상태, 플랜, 실행 큐가 새 캐릭터 기준으로 동기화된다.
...
14. 키보드만으로 열기, 카드 선택, 적용, 취소가 가능하다.
