                ...
                ${detail.images.length ? `
                    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
                        ${detail.images.map(image => {
                            ...
                            const displayName = image.name || fileName;
                            const hasDistinctDisplayName = displayName !== fileName;
                            const imageAlt = hasDistinctDisplayName ? `${displayName} (${fileName})` : fileName;
                            return `
                            <button type="button" data-image-url="${escapeHtml(image.url)}" data-image-title="${escapeHtml(`${detail.name} · ${displayName}`)}" data-image-path="${escapeHtml(image.path || fileName)}" class="group relative flex w-full flex-col items-center overflow-hidden rounded-lg border border-gray-100 p-2 transition hover:border-indigo-200 hover:bg-gray-50 dark:border-gray-700 dark:hover:border-indigo-500 dark:hover:bg-gray-700 sm:p-3">
                                <span class="block h-24 w-full overflow-hidden rounded border border-gray-200 bg-gray-100 shadow-sm dark:border-gray-600 dark:bg-gray-800 sm:h-32">
                                    <img src="${escapeHtml(image.url)}" alt="${escapeHtml(imageAlt)}" loading="lazy" class="h-full w-full object-cover">
                                </span>
                                ${hasDistinctDisplayName
                                    ? `<span class="mt-2 flex w-full flex-col items-center overflow-hidden text-center">
                                        <span class="block w-full truncate text-[10px] font-bold text-gray-700 group-hover:text-indigo-700 dark:text-gray-200 dark:group-hover:text-indigo-400 sm:text-xs" title="${escapeHtml(displayName)}">${escapeHtml(displayName)}</span>
                                        <span class="block w-full truncate text-[9px] text-gray-500 dark:text-gray-400 sm:text-[10px]" title="${escapeHtml(fileName)}">(${escapeHtml(fileName)})</span>
                                    </span>`
                                    : `<span class="mt-2 block w-full truncate text-center text-[10px] font-medium text-gray-600 group-hover:text-indigo-700 dark:text-gray-300 dark:group-hover:text-indigo-400 sm:text-xs" title="${escapeHtml(fileName)}">${escapeHtml(fileName)}</span>`}
                            </button>`;
                        }).join('')}
                    </div>` : `
